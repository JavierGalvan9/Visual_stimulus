...
class DriftingGratings:
    ...
    def __init__(self, orientation, TF=2, SF=0.04, reverse=False, init_screen_dur=0.5,
        ...
        self.second_transition = int((self._init_screen_dur+self._visual_flow_dur)/self._dt)
        # total movie time
        self.total_time = int(
    ...
    def from_angle_to_gratings(self, angles):
        ...
        """
        gratings = (((np.sin(angles)+1)/2) * (self._max_value-self._min_value) + self._min_value)*self._contrast
        return gratings
    ...
    def initial_frame(self, gray_screen=True):
        ...
        if not gray_screen:
            if self._orientation in [0, 180]:
                ...
                init_frame[:, :] += np.linspace(0, 2*np.pi*self.n_gratings, self._width)
            elif self._orientation in [90, 270]:
                self.n_gratings = self._SF * self._height
                init_frame = np.transpose(np.transpose(init_frame[:, :])+np.linspace(0, 2*np.pi*self.n_gratings, self._height))
...
#                 small_diagonal = np.sqrt(2*(min([self._height, self._width]))**2)
    #             self._n_gratings = self._SF * self._small_diagonal
                n_horizontal_gratings = (self._SF/np.cos(np.deg2rad(self._orientation))) * self._width
    #             n_vertical_gratings = (self._SF/np.sin(np.deg2rad(self._orientation))) * self._height
                init_frame[0, :] += np.linspace(0, 2*np.pi*n_horizontal_gratings,
                                                       self._width)/2
                delta_y = 2*np.pi*self._SF/np.sin(np.deg2rad(self._orientation))/2
                for i in range(1, self._height):
                    init_frame[i, :] = init_frame[i-1, :] + delta_y
            else:
                print('The orientation selected is not within the available ones. Try another one!')
    ...
    def gratings(self):
        ...
        self.visual_stimulus[:self.first_transition, :, :] = self.from_angle_to_gratings(init_frame)
        
        # Second period: drifting gratings
        angle_shift = np.zeros([self.visual_flow_time, self._height, self._width])
        ...
        self.visual_stimulus[self.first_transition:self.second_transition, :, :] = self.from_angle_to_gratings(angle_shift)
        
        # Third period: static frame
        if self._end_gray_screen:
            self.visual_stimulus[self.second_transition:, :, :] = self.from_angle_to_gratings(self.initial_frame(gray_screen=self._end_gray_screen))
        else:
            self.visual_stimulus[self.second_transition:, :, :] = self.from_angle_to_gratings(current_angles)
    ...
    def reversed_gratings(self):
        ...
        first_angle_shift = np.zeros([self.init_screen_time, self._height, self._width])
        ...
        self.visual_stimulus[:self.first_transition, :, :] = self.from_angle_to_gratings(first_angle_shift)
        
        # Second period: static frame
        if self._init_gray_screen:
            self.visual_stimulus[self.first_transition:self.second_transition, :, :] = self.from_angle_to_gratings(self.initial_frame())
        else:
            self.visual_stimulus[self.first_transition:self.second_transition, :, :] = self.from_angle_to_gratings(current_angles)

        # Third period: drifting gratings
        second_angle_shift = np.zeros([self.end_screen_time, self._height, self._width])
        ...
        for i in range(self.end_screen_time):
            if self._orientation in [180, 270]:
                ...
                second_angle_shift[i, :, :] -= i*self._dt*self._TF*2*np.pi
        self.visual_stimulus[self.second_transition:, :, :] = self.from_angle_to_gratings(second_angle_shift)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--temporal_frequency', type=int, default=2)
    ...
    parser.add_argument('--no-init_gray_screen', dest='init_gray_screen', action='store_false')
    parser.set_defaults(init_gray_screen=False)
    parser.add_argument('--end_gray_screen', action='store_true')
    parser.add_argument('--no-end_gray_screen', dest='end_gray_screen', action='store_false')
